...
def _load_fixture_lookup(conn, season_year: int) -> dict[tuple[date, int, int], tuple[str, datetime]]:
    query = """
        SELECT fixture_id, date, home_team_id, away_team_id
        FROM fixtures_historical
        WHERE season = %s
    """
    lookup: dict[tuple[date, int, int], tuple[str, datetime]] = {}
    with conn.cursor() as cur:
        cur.execute(query, (season_year,))
        for fixture_id, fixture_date, home_id, away_id in cur.fetchall():
            if fixture_date is None:
                continue
            key = (fixture_date.date(), int(home_id), int(away_id))
            lookup[key] = (fixture_id, fixture_date)
    return lookup


def _collect_odds(row: dict[str, str]) -> dict[str, list[float]]:
...
def _process_season(csv_path: Path, season: str, conn) -> None:
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            home_id = _stable_int(home_name)
            away_id = _stable_int(away_name)
            fixture_key = (match_date, home_id, away_id)
            ...
            odds = _collect_odds(row)
            averaged = {
                selection: _average(values) for selection, values in odds.items()
            }
            if not any(value is not None for value in averaged.values()):
                missing_rows += 1
                continue

            captured_at = fixture_datetime - timedelta(hours=1)
            for selection, value in averaged.items():
                if value is None:
                    continue
                payloads.append(
                    {
