...
from intelligence.match_context import MatchContextBuilder
...
def generate_round(
    ...
    # 8. Build match contexts (always)
    print("\nBuilding match contexts...")
    ctx_builder = MatchContextBuilder(league_id=league_id)
    narrator_instance = None
    if narrative and not context_only:
        try:
            from intelligence.narrator import Narrator
            narrator_instance = Narrator(model=llm_model)
            print(f"  Narrator ready ({llm_model})")
        except Exception as e:
            print(f"  Narrator init failed: {e}")
    ...
    for report in reports:
        ...
        try:
            ...
            # Add team names to match block (narrator needs them)
            ctx["match"]["home_team"] = home
            ctx["match"]["away_team"] = away

            with open(match_dir / "context.json", "w") as f:
                json.dump(ctx, f, indent=2, default=str, ensure_ascii=False)
        except Exception as e:
            print(f"  Context failed for {home} vs {away}: {e}")
            continue

        # Generate narrative (if requested)
        if narrator_instance:
            try:
                narrative_path = match_dir / "narrative.json"
                result = narrator_instance.generate(
                    ctx,
                    cache_path=narrative_path,
                    regenerate=regenerate,
                )
                cost = result.get("cost_estimate", 0)
                tokens = result.get("tokens_used", 0)
                print(f"  {home} vs {away}: {tokens} tokens (${cost:.4f})")
            except Exception as e:
                print(f"  Narrative failed for {home} vs {away}: {e}")

    ctx_builder.close()
    print("  Contexts built for all matches")
...
def main() -> int:
    ...
    parser.add_argument("--narrative", action="store_true",
                        help="Generate LLM narratives (requires OPENAI_API_KEY)")
    parser.add_argument("--llm-model", default="gpt-4o-mini",
                        help="LLM model for narrative generation (default: gpt-4o-mini)")
    parser.add_argument("--regenerate", action="store_true",
                        help="Force regenerate narratives (skip cache)")
    parser.add_argument("--context-only", action="store_true",
                        help="Build context.json only, skip LLM")
