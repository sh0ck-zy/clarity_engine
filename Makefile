...
.PHONY: sync kg round board refresh backfill-players status track \
        sync-all kg-all backfill-all status-all odds odds-all

# ── Single league ──────────────────────────────────────────────

# Scrape latest match data
sync:
	$(PYTHON) scripts/sync_fotmob_playwright.py --league $(LEAGUE) --details --finished-only
...
backfill-players:
	$(PYTHON) scripts/sync_fotmob_playwright.py --league $(LEAGUE) --extract-players-from-raw
	$(PYTHON) scripts/sync_fotmob_playwright.py --league $(LEAGUE) --backfill-players
	$(PYTHON) scripts/populate_kg_states.py --league-id $(LEAGUE_ID)

# Quick data health check
status:
	@$(PYTHON) -c "import psycopg2; c=psycopg2.connect('dbname=clarity_football'); cur=c.cursor(); \
	cur.execute(\"SELECT status, COUNT(*) FROM fotmob_matches WHERE league_id=$(LEAGUE_ID) GROUP BY status\"); \
	print('Matches:'); [print(f'  {r[0]}: {r[1]}') for r in cur.fetchall()]; \
	cur.execute(\"SELECT COUNT(DISTINCT fp.fotmob_match_id) FROM fotmob_player_performances fp JOIN fotmob_matches fm ON fm.fotmob_match_id=fp.fotmob_match_id WHERE fm.league_id=$(LEAGUE_ID)\"); \
...
sync-all:
	@for pair in $(ALL_LEAGUES); do \
		code=$${pair%%:*}; id=$${pair##*:}; \
		echo "\n=== Syncing $$code (id=$$id) ==="; \
		$(PYTHON) scripts/sync_fotmob_playwright.py --league $$code --details --finished-only; \
	done
...
backfill-all:
	@for pair in $(ALL_LEAGUES); do \
		code=$${pair%%:*}; id=$${pair##*:}; \
		echo "\n=== Backfill $$code (id=$$id) ==="; \
		$(PYTHON) scripts/sync_fotmob_playwright.py --league $$code --extract-players-from-raw; \
		$(PYTHON) scripts/sync_fotmob_playwright.py --league $$code --backfill-players; \
		$(PYTHON) scripts/populate_kg_states.py --league-id $$id; \
	done
...
status-all:
	@for pair in $(ALL_LEAGUES); do \
		...
		echo "\n=== $$code (id=$$id) ==="; \
		$(PYTHON) -c "import psycopg2; c=psycopg2.connect('dbname=clarity_football'); cur=c.cursor(); \
		cur.execute(\"SELECT status, COUNT(*) FROM fotmob_matches WHERE league_id=$$id GROUP BY status\"); \
		print('Matches:'); [print(f'  {r[0]}: {r[1]}') for r in cur.fetchall()]; \
		cur.execute(\"SELECT COUNT(DISTINCT fp.fotmob_match_id) FROM fotmob_player_performances fp JOIN fotmob_matches fm ON fm.fotmob_match_id=fp.fotmob_match_id WHERE fm.league_id=$$id\"); \
		print(f'With player data: {cur.fetchone()[0]}'); \
